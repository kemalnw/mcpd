...
func RegisterSearch(server *mcp.Server, manager *searchmgr.Manager, auditStore *audit.Store) {
	...
	mcp.AddTool(server, tool("start_search", "Search files or file contents", "Use this when a file path is unknown, when locating filenames across a tree, or when searching text across many files. searchType=files searches names; searchType=content searches file contents and returns each match with the full source line in text plus match, line, and Unicode-aware column/end_column span. When the user names a project or repository but its exact path is unknown, set pathHint to that project/repository name so MCPD can prioritize likely workspace roots and avoid broad duplicate searches. The search runs progressively and returns a sessionId; use get_more_search_results with that ID when more results are needed. Prefer list_directory for browsing one known directory and read_file when the exact file is already known.", toolHints{readOnly: true}), audited(auditStore, "start_search", t.start))
