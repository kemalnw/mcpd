...
func RegisterProcess(server *mcp.Server, manager *processmgr.Manager, auditStore *audit.Store) {
	...
	mcp.AddTool(server, tool("start_process", "Run a shell command", "Use this for shell commands, builds, tests, package managers, Git, service inspection, and other terminal work. For repository-scoped work, pass cwd instead of embedding `cd <path> &&`. For non-PTY commands, set separate_streams=true only when stdout/stderr identity matters; default merged output remains more compact. The call returns on exit, prompt, or timeout without killing a running process. Continue the same PID with read_process_output or interact_with_process. For commands that must not execute twice after a lost response, supply a unique idempotency_key and reuse it on transport retries.", toolHints{destructive: true, openWorld: true}), audited(auditStore, "start_process", t.start))
...
type StartProcessInput struct {
	...
	IdempotencyKey  string `json:"idempotency_key,omitempty" jsonschema:"optional retry-safety key; retries with the same execution request return the original PID/result instead of starting a duplicate process"`
}
...
type InteractWithProcessInput struct {
	...
	OperationKey  string `json:"operation_key,omitempty" jsonschema:"optional retry-safety key for this stdin write; the same key+input is written at most once for the managed session"`
}
...
type PIDInput struct {
	...
	ExpectedStartTicks uint64 `json:"expected_start_ticks,omitempty" jsonschema:"optional Linux /proc starttime identity from list_processes; rejects PID reuse before signaling"`
}
...
func (t *ProcessTools) start(ctx context.Context, in StartProcessInput) (processmgr.StartResult, error) {
	return t.manager.Start(ctx, processmgr.StartRequest{Command: in.Command, CWD: in.CWD, Shell: in.Shell, TimeoutMS: in.TimeoutMS, PTY: processmgr.PTYMode(in.PTY), SeparateStreams: in.SeparateStreams, IdempotencyKey: in.IdempotencyKey})
}
...
func (t *ProcessTools) interact(ctx context.Context, in InteractWithProcessInput) (processmgr.InteractResult, error) {
	...
	return t.manager.Interact(ctx, processmgr.InteractRequest{PID: in.PID, Input: in.Input, TimeoutMS: in.TimeoutMS, WaitForPrompt: wait, RawInput: in.RawInput, OperationKey: in.OperationKey})
}
...
func (t *ProcessTools) killProcess(_ context.Context, in PIDInput) (TerminateOutput, error) {
	if err := processmgr.KillSystemProcess(in.PID, in.ExpectedStartTicks); err != nil {
		return TerminateOutput{}, err
	}
